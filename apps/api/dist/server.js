...
app.use((0, cors_1.default)({
    origin: function (origin, callback) {
        if (!origin)
            return callback(null, true); // allow same-origin or curl
...
app.post("/webhooks/stripe", express_1.default.raw({ type: "application/json" }), (req, res) => {
    try {
...
app.post("/tenants/sync", async (req, res) => {
    if (!isAuthorized(req))
    ...
    try {
        if (!process.env.DATABASE_URL)
            return res.status(200).json({ ok: true, tenant: { id: tenantId, name: name || tenantId, ephemeral: true } });
...
const stripe = STRIPE_SECRET_KEY ? new stripe_1.default(STRIPE_SECRET_KEY, { apiVersion: "2023-10-16" }) : null;
...
const AgentCreateSchema = zod_1.z.object({
    name: zod_1.z.string().min(2).max(80),
    tenantId: zod_1.z.string().optional(),
    channel: zod_1.z.enum(["chat", "voice"]).default("chat"),
...
app.patch("/agents/:id", async (req, res) => {
    ...
    try {
        ...
        const updated = {
            ...
            status: status ?? agentsMem[idx].status,
...
app.delete("/agents/:id", async (req, res) => {
    ...
    try {
        ...
        for (let i = agentsMem.length - 1; i >= 0; i--) {
            if (agentsMem[i].id === id && agentsMem[i].tenantId === tenantId)
                agentsMem.splice(i, 1);
        }
...
function validateTwilioRequest(req) {
    try {
        ...
        const base = API_PUBLIC_URL || `${req.protocol || "http"}://${req.get("host") || "localhost"}`;
...
const callsMem = [];
const callEventsMem = [];
const transcriptsMem = [];
const phoneNumbersMem = [];
// --- Helpers ---
...
app.post("/calls/initiate", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.warn("[initiate] failed to create call record:", e?.message || e);
        }
...
app.post("/twilio/voice/inbound", async (req, res) => {
    ...
    try {
        ...
        console.warn("[inbound] failed to create call record:", e?.message || e);
    }
...
app.post("/twilio/voice/gather", (req, res) => {
...
app.post("/twilio/voice/status", (req, res) => {
    ...
    (async () => {
        try {
            ...
            console.warn("[status] update failed:", e?.message || e);
        }
    })();
    res.json({ ok: true });
});
...
app.get("/admin/phone-numbers", requireAdmin, async (_req, res) => {
    if (!ensureDbConfigured(res))
        return;
    try {
        const prisma = getPrisma();
...
app.post("/admin/phone-numbers", requireAdmin, async (req, res) => {
    if (!ensureDbConfigured(res))
        return;
...
app.delete("/admin/phone-numbers/:id", requireAdmin, async (req, res) => {
    if (!ensureDbConfigured(res))
        return;
    const id = String(req.params.id || "");
    if (!id)
...
const usageMem = [];
app.get("/admin/usage", requireAdmin, async (_req, res) => {
    if (process.env.DATABASE_URL) {
        try {
            const prisma = getPrisma();
...
app.patch("/admin/flags", requireAdmin, (req, res) => {
    try {
        const body = (req.body || {});
        let changed = false;
